...
- Order item quantity validation now rejects fractional values; upload MIME sniffing uses the latest `file-type` release via runtime ESM loading; reservation expiry now uses a secret-protected internal cron endpoint instead of an in-process timer.
