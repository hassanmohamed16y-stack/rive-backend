...
export class OrdersController {
  ...
  private getGuestAccessToken(req: any): string | undefined {
    return typeof req.headers?.['x-order-access-token'] === 'string'
      ? req.headers['x-order-access-token']
      : undefined;
  }

  private assertOrderAccess(order: { userId: string | null; guestAccessToken?: string | null }, req: any) {
  ...
  async findOne(@Param('orderNumber') orderNumber: string, @Req() req: any) {
    const guestAccessToken = this.getGuestAccessToken(req);
  ...
  async cancel(@Param('orderNumber') orderNumber: string, @Req() req: any) {
    const guestAccessToken = this.getGuestAccessToken(req);
    const order = await this.ordersService.findOne(orderNumber, guestAccessToken);
    this.assertOrderAccess(order, req);
