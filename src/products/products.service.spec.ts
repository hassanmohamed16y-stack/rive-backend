...
describe("ProductsService public visibility and pagination", () => {
  function createService() {
    const prisma = {
      product: {
        findMany: jest
          .fn()
          .mockResolvedValue([
            { id: "active-product", status: ProductStatus.ACTIVE },
          ]),
    ...
    return {
      service: new ProductsService(prisma as any, auditLogService as any),
      prisma,
      auditLogService,
    };
  }
  ...
  it("limits public lists to ACTIVE products and returns pagination metadata", async () => {
    ...
    await expect(
      service.findAll(undefined, { page: 2, limit: 10 }),
    ).resolves.toEqual({
  ...
  it("does not expose a DRAFT product through the public slug lookup", async () => {
    ...
    await expect(service.findOneBySlug("draft-product")).rejects.toBeInstanceOf(
      NotFoundException,
    );
  ...
  it("allows the protected admin path to list and retrieve all statuses", async () => {
    ...
    await expect(
      service.findOneBySlug("draft-product", true),
    ).resolves.toMatchObject({ status: ProductStatus.DRAFT });
    await expect(
      service.findByIdForAdmin("draft-product"),
    ).resolves.toMatchObject({ status: ProductStatus.DRAFT });
    expect(prisma.product.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(prisma.product.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: { slug: "draft-product" } }),
    );
    expect(prisma.product.findUnique).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id: "draft-product" } }),
    );
  });
});
...
describe("ProductsService variant and image administration", () => {
  ...
  it("adds a variant and records an audit log entry", async () => {
    ...
    await expect(
      service.addVariant("product-1", dto, "admin-1"),
    ...
    expect(auditLogService.record).toHaveBeenCalledWith(
      expect.objectContaining({ action: "product.variant.create" }),
    );
  });
  ...
  it("sets an absolute stock value (not a decrement) when updating a variant", async () => {
    ...
    await expect(
      service.updateVariant("product-1", "variant-1", dto, "admin-1"),
    ).resolves.toMatchObject({ stock: 42 });
    ...
    expect(auditLogService.record).toHaveBeenCalledWith(
      expect.objectContaining({ action: "product.variant.update" }),
    );
  });
  ...
  it("rejects a concurrent update with ConflictException when updatedAt no longer matches", async () => {
    ...
    const dto = { stock: 42, expectedUpdatedAt: now.toISOString() } as any;
    await expect(
      service.updateVariant("product-1", "variant-1", dto),
  ...
  it("prevents deleting a variant that has existing order items", async () => {
    ...
    await expect(
      service.removeVariant("product-1", "variant-1"),
    ).rejects.toBeInstanceOf(ConflictException);
  });
  ...
  it("deletes a variant with no order history and records an audit log entry", async () => {
    ...
    await expect(
      service.removeVariant("product-1", "variant-1", "admin-1"),
    ).resolves.toEqual({ success: true });
    expect(auditLogService.record).toHaveBeenCalledWith(
      expect.objectContaining({ action: "product.variant.delete" }),
    );
  });
  ...
  it("adds a product image and records an audit log entry", async () => {
    ...
    await expect(
      service.addImage("product-1", dto, "admin-1"),
    ).resolves.toMatchObject({ id: "image-1" });
    expect(auditLogService.record).toHaveBeenCalledWith(
      expect.objectContaining({ action: "product.image.create" }),
    );
  });
  ...
  it("removes a product image and records an audit log entry", async () => {
    ...
    await expect(
      service.removeImage("product-1", "image-1", "admin-1"),
    ).resolves.toEqual({ success: true });
    expect(auditLogService.record).toHaveBeenCalledWith(
      expect.objectContaining({ action: "product.image.delete" }),
    );
  });
  ...
  it("throws NotFoundException when removing an image that does not belong to the product", async () => {
    ...
    await expect(
      service.removeImage("product-1", "missing-image"),
    ).rejects.toBeInstanceOf(NotFoundException);
  });
...
describe("ProductsService update/archive/create error mapping", () => {
  ...
  it("throws ConflictException (not NotFoundException) when updating a product to a duplicate slug (P2002)", async () => {
    ...
    await expect(
      service.update("product-1", { slug: "duplicate-slug" } as any),
    ).rejects.toBeInstanceOf(ConflictException);
  });
  ...
  it("rethrows unexpected errors from update without masking them as NotFoundException", async () => {
    ...
    await expect(
      service.update("product-1", { name: "x" } as any),
