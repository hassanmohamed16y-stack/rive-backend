...
describeWithDatabase("Stripe payment flow with PostgreSQL", () => {
  ...
  beforeAll(async () => {
    ...
    const category = await prisma.category.create({
      data: { name: `Payments ${suffix}`, slug: `payments-${suffix}` },
    });
    ...
    const variants = await Promise.all(
      ["PAID", "EXPIRED"].map((sku, index) =>
        prisma.productVariant.create({
          data: {
            productId,
            sku: `PAYMENT-${sku}-${suffix}`,
            size: index === 0 ? Size.S : Size.M,
            price: 25,
            stock: 1,
          },
        }),
  ...
  it("reserves stock, creates a session, and marks the order paid once from a verified webhook", async () => {
    ...
    await expect(
      paymentService.createCheckoutSession(order.id, {
        guestAccessToken: order.guestAccessToken,
      }),
    ).resolves.toMatchObject({ sessionId: "cs_paid" });
    await expect(
      paymentService.handleWebhook(Buffer.from("{}"), "valid"),
    ).resolves.toMatchObject({ status: OrderStatus.PAID });
    await expect(
      paymentService.handleWebhook(Buffer.from("{}"), "valid"),
    ).resolves.toMatchObject({ message: "Webhook already processed." });
      ...
      checkout: {
        sessions: {
          create: jest
            ...
            .mockResolvedValue({
              id: "cs_expired",
              url: "https://checkout.stripe.test/cs_expired",
            }),
        },
      },
      webhooks: {
