...
describe('AuditLogService', () => {
  ...
  it('handles write errors gracefully without crashing the calling flow', async () => {
    ...
    expect(loggerSpy).toHaveBeenCalledWith(
      expect.stringContaining('Failed to record audit log for Test:test-1 (action=test.action): Database connection failed'),
      expect.any(String),
    );
  ...
  it('handles non-Error throwables during write gracefully', async () => {
    ...
    expect(loggerSpy).toHaveBeenCalledWith(
      expect.stringContaining('Failed to record audit log for Test:test-1 (action=test.action): String error'),
      undefined,
    );
  ...
  it('omits changes and logs warning if changes payload is not JSON-serializable', async () => {
    ...
    expect(loggerSpy).toHaveBeenCalledWith(
      expect.stringContaining('Audit log changes payload for Test:test-2 (action=test.circular) is not JSON-serializable'),
    );
