...
describe('CategoriesController', () => {
  ...
  describe('findAll', () => {
    it('returns categories with mapped productCount', async () => {
      ...
      const query: ListCategoriesQueryDto = { isFeatured: 'true', page: 1, limit: 20 };
  ...
  describe('update', () => {
    it('updates a category with the authenticated user ID', async () => {
      ...
      expect(categoriesService.update).toHaveBeenCalledWith('cat-2', dto, 'admin-123');
  ...
  describe('remove', () => {
    it('deletes a category with the authenticated user ID', async () => {
      const req = { user: { id: 'admin-123', role: 'ADMIN' } } as any;
      categoriesService.remove.mockResolvedValue({ message: 'Category deleted successfully' } as any);

      const res = await controller.remove('cat-2', req);

      expect(categoriesService.remove).toHaveBeenCalledWith('cat-2', 'admin-123');
