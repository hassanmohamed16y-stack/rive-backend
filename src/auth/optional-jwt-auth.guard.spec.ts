...
describe('OptionalJwtAuthGuard', () => {
  ...
  describe('canActivate', () => {
    it('returns true directly without calling Passport when no Authorization header is present', () => {
      ...
      const superCanActivateSpy = jest.spyOn(Object.getPrototypeOf(OptionalJwtAuthGuard.prototype), 'canActivate');
  ...
  describe('handleRequest', () => {
    it('returns user when user is present and no error occurred', () => {
      const user = { userId: 'user-123', email: 'user@example.com', role: 'CUSTOMER' };
    ...
    it('throws custom error if provided to handleRequest', () => {
      ...
      expect(() => guard.handleRequest(err, false, null, {} as ExecutionContext)).toThrow(err);
    });

    it('throws UnauthorizedException when user is missing/false and no error was provided (invalid token path)', () => {
      expect(() => guard.handleRequest(null, false, null, {} as ExecutionContext)).toThrow(
