describe("JWT_SECRET hardening outside local development/test", () => {
  ...
  it.each(["production", "staging", "qa"])(
    "throws when JWT_SECRET is missing for NODE_ENV=%s",
    (nodeEnv) => {
  ...
  it.each(["production", "staging", "qa"])(
    "does not throw when JWT_SECRET is set for NODE_ENV=%s",
    (nodeEnv) => {
      expect(
        loadAuthModuleWithEnv(
          nodeEnv,
          "a-secure-jwt-secret-that-is-long-enough",
        ),
      ).not.toThrow();
    },
  );
  ...
  it.each(["development", "test"])(
    "does not throw when JWT_SECRET is missing for local NODE_ENV=%s",
    (nodeEnv) => {
