...
describe("AuthService", () => {
  ...
  it("rotates refresh tokens and revokes the old token after use", async () => {
    ...
    expect(prisma.refreshToken.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id: "refresh-1", revokedAt: null } }),
    );
  ...
  it("forgot-password always returns the same generic message, even for unknown emails", async () => {
    ...
    const storedToken = (prisma.user.update as jest.Mock).mock.calls[0][0].data
      .passwordResetToken;
    const [, sentToken] = (emailService.sendPasswordResetEmail as jest.Mock)
      .mock.calls[0];
  ...
  it("resets the password with a valid token and revokes refresh tokens", async () => {
    ...
    expect(prisma.user.findUnique).toHaveBeenCalledWith({
      where: { passwordResetToken: hashToken("valid-token") },
    });
    expect(prisma.refreshToken.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { userId: "user-1", revokedAt: null },
  ...
  it("rejects an expired password reset token", async () => {
    ...
    await expect(
      service.resetPassword("valid-token", "NewStrongPass123!"),
  ...
  it("rejects an unknown password reset token", async () => {
    ...
    await expect(
      service.resetPassword("missing", "NewStrongPass123!"),
